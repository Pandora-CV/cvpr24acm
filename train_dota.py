...
#parser.add_argument("--heads", default={'hm': 15,'wh': 2 ,'reg': 2, 'theta':4})
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch,args.epochs) :
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            if 1:
                center_loss, scale_loss, offset_loss, theta_loss = model(
                    {'img': img, 'label': label, 'heatmap_t': heatmap_t, 'theta': theta})
        ...
        if args.rank == 0 and (epoch + 1) % args.save_interval == 0:
            ...
            print("Saving...")
            torch.save(model.state_dict(), f"checkpoint/dota/{args.box_loss}_{args.coder}_{args.coder_cfg}_{args.coder_mode}_{args.batch_size}%d.pth" % (epoch+1))
